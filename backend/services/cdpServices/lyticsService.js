...
const LYTICS_DOC_BASE_URL = 'https://docs.lytics.com';

const fetchDocumentation = async () => {
  try {
    const response = await axios.get(LYTICS_DOC_BASE_URL);
    ...
    for (const link of docsToProcess) {
      await processDocumentationPage(`${LYTICS_DOC_BASE_URL}${link}`, 'lytics');
    }
    
    console.log(`Processed ${docsToProcess.length} Lytics documentation pages`);
    return true;
  } catch (error) {
...
const searchDocumentation = async (query) => {
  try {
    ...
    console.error('Error searching Lytics documentation:', error);
    throw error;
  }
};
