...
const ZEOTAP_DOC_BASE_URL = 'https://docs.zeotap.com/home/en-us';

const fetchDocumentation = async () => {
  try {
    const response = await axios.get(ZEOTAP_DOC_BASE_URL);
    const $ = cheerio.load(response.data);
    
    const docLinks = [];
    $('a[href^="/home/"]').each((_, element) => {
    ...
    for (const link of docsToProcess) {
      await processDocumentationPage(`https://docs.zeotap.com${link}`, 'zeotap');
    }
    
    console.log(`Processed ${docsToProcess.length} Zeotap documentation pages`);
    return true;
  } catch (error) {
...
// Search for specific information in Zeotap documentation
const searchDocumentation = async (query) => {
  try {
    ...
    console.error('Error searching Zeotap documentation:', error);
    throw error;
  }
};
